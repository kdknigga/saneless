...
- [x] **DLVR-06**: A `.dockerignore` allow-list keeps config files, secrets, `.planning/`, and tests out of the build context [M-31]
- [x] **DLVR-07**: The container runs as a non-root user, base images are pinned by digest, and a `WORKDIR` is set so relative writes do not land in `/` [N-26]
...
- [x] **DLVR-09**: `.gitignore` no longer blanket-ignores `*.png`; only specific screenshot directories are excluded [N-32]
...
| DLVR-06 | Phase 31 — Delivery, Identity, and Documentation Accuracy | Complete |
| DLVR-07 | Phase 31 — Delivery, Identity, and Documentation Accuracy | Complete |
