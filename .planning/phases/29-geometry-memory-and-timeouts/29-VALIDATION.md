...
- [ ] `tests/fake_sane.py` — Event-gated blocking read (unblock-by-return with a truncated page,
...
- [ ] `tests/conftest.py` — `spooling()` / `spooling_in_turn()` `side_effect` factories, and a
      `StubScannerBackend` base for the existing stub scanner classes
- [ ] subprocess child script for the process-exit proof (inline source constant written to
      `tmp_path`)
