...
- **D-11: `classify_source()` becomes the only source-classification rule, and the resulting C-06 routing correction lands here — intentionally.** Success criterion 2 is unconditional: "it is the only classification rule in the codebase." Today two rules disagree — `sane_backend._is_adf_source` (`:158-160`) tests `"adf" in source.lower()`, while `auto_profiles.source_to_slug` (`:47-58`) also recognises `"document feeder"` and `"feeder"`. One rule cannot produce two answers, so satisfying the criterion necessarily changes what `sane_backend` does with `"Automatic Document Feeder"`: it starts taking the `multi_scan()` path and returns all the pages instead of one.
  ...
  - **`SourceKind.UNKNOWN` keeps today's routing** — it falls to the single-page branch. C-06 also floats a safer default ("treat anything that is not the flatbed entry as multi-page"); that is a real bet about unseen scanners and it stays **Phase 24's** call.

  - **AMENDED 2026-09-10, after execution and code review (CR-01).** As originally written, D-11 confined the change to "names that are unambiguously feeders". The shipped `classify_source` tests `"duplex"` *before* the feeder tokens, so names carrying `"duplex"` but no feeder token — this project's own `"Manual Duplex"`, Fujitsu's `"Card Duplex"` — also classify as `FEEDER_DUPLEX` and route to `multi_scan()`. The code review flagged this as exceeding D-11. **The user reviewed it and chose to keep the shipped behaviour**, on this evidence:
    - `docs/how-to/set-up-adf-duplex.md` prescribes exactly `source = "Manual Duplex"`, tells the user to load pages **in the ADF**, and promises "saneless scans **all front sides** (Pass A)".
    - The pre-phase rule was `"adf" in source.lower()`, which is **False** for `"manual duplex"` — so that documented config routed to `snap()` and produced **one page per pass**. The documented workflow was already broken.
    - Routing it to `multi_scan()` makes the documented behaviour true for the first time.
