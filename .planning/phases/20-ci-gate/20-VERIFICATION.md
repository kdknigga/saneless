...
| GitHub ruleset `kdknigga/scanless/rulesets/22777879` | `required_status_checks` for `lint`+`test`, `enforcement: active`, targets `refs/heads/master` | ✓ VERIFIED | Confirmed via direct `gh api` read in this verification session. |
...
| Ruleset live and active | `gh api repos/kdknigga/scanless/rulesets/22777879` | `enforcement: active`, `required_status_checks` for `lint`+`test`, `bypass_actors` non-empty | ✓ PASS |
