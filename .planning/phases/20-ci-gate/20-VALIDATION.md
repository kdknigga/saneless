...
| CI-01 | All five checks pass locally | smoke | `uv run ruff check . && uv run ruff format --check . && uv run ty check && uv run pyrefly check src tests && uv run pytest -m "not browser"` | ✅ | ⬜ pending |
...
| CI-01 (D-16) | Bumped checkers pass clean | smoke | `uv run ty check && uv run pyrefly check src tests` | ✅ (pyrefly already 0; ty needs 3 fixes) | ⬜ pending |
