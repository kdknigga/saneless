...
| T-20-13 | Elevation of Privilege | mitigate | CLOSED | Verified by execution, not by reading the file. Run `34493178468` (head `7e977b4`) job steps: `lint` ran `uv run ruff check .`, `uv run ruff format --check .`, `uv run ty check`, `uv run pyrefly check src tests`; `test` ran `uv run pytest -m "not browser"`. **All five, all `success`, as real steps.** Grep of `ci.yml` for `continue-on-error`, `\|\| true`, `pull_request_target`, `permissions:.*write`, `secrets.` → **0 matches**. |
