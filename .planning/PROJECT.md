...
- ✓ Exception translation: every SANE, httpx, img2pdf (all seven classes) and TOML failure becomes a saneless exception carrying its original message and cause; the CLI prints one line and exits with one table — 0 ok, 1 scan, 2 config/setup (incl. missing python-sane, unusable job database, serve start failure), 3 Paperless, 4 PDF assembly, 5 unexpected bug, 130 cancelled — with no traceback unless `-v`; zero pages say "No pages were scanned" and "All pages were blank" only when detection removed them; an explicit flip-prompt abort is a terminal CANCELLED job (muted styling, INFO log) while timeouts, prompt failures and shutdowns stay ERROR; every job failure is logged with `exc_info`; uploads retry every transient transport error and 5xx, polling survives transport errors, credentials never reach messages or logs; new troubleshooting how-to with doc-truth exit-code tests (EXC-01..05) — Phase 28
- ✓ Appliance layer: one shared three-state check registry behind `saneless doctor` (non-zero on a placeholder token or any red check, `[SKIP]` for a check nothing probed) and a cached status strip that stays fast with the scanner unplugged, skips the scanner during a scan, never contends with the exclusive scanner gate, and whose own failing poll ends itself instead of overwriting the scan-progress slot; page counts on every terminal job; plain-language errors with a next step and a collapsed technical disclosure; human profile labels and descriptions, feeder-first; queue position; owner-only Continue/Abort flip prompt; help text, thumb-friendly tag picker, read-only-mount notice; docs for every new surface (APPL-01..12) — Phase 30 (36 plans: 19 shipped + 17 across three gap-closure rounds; final verification 5/5)
...
---
*Last updated: 2026-09-18 — Phase 30 complete: appliance layer (36 plans; three gap-closure rounds driven by deep code review; round-4 review 0 critical / 3 warning / 3 info carried as follow-up debt; verification passed 5/5)*
