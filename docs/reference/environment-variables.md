...
A path that does not exist, or one that is a directory where `SSL_CERT_FILE` expects a file, is not a silent fallback to the OS trust store either: saneless refuses at startup with `Paperless error: Could not build the TLS trust store for Paperless at <url>: <OS error text>; check SSL_CERT_FILE and SSL_CERT_DIR`. In Docker that is what a bind mount whose host file is missing produces, because Docker creates a directory at an absent bind-mount source. Create the file on the host and recreate the container.
